...
const MentalHealth = ({navigation}) => {
    ...
    React.useEffect(() => {
        const collectionRef = collection(db, 'MentalHealth');
    ...
    React.useEffect(() => {
        const collectionRef = collection(db, 'MentalHealth1');
        const q = query(collectionRef, orderBy('createdAt', 'desc'));
    ...
    React.useEffect(() => {
        const collectionRef = collection(db, 'MentalHealth2');
        const q = query(collectionRef, orderBy('createdAt', 'desc'));
    ...
    return (
        ...
        <View>
            ...
            <Text style={styles.title1}>Mental Health</Text>
            {products.map(product => <EditMentalHealth key={product.id} {...product} />)}
            <TouchableOpacity onPress={() => navigation.navigate(AddMentalHealth)} style={styles.button}>
