...
import EditEmotionalHealth from './EditEmotionalHealth';


MaterialCommunityIcons.loadFont();

const EmotionalHealth = ({navigation}) => {

    const [products, setProducts] = React.useState([]);

    React.useEffect(() => {
        const collectionRef = collection(db, 'EmotionalHealth');
    ...
    React.useEffect(() => {
        const collectionRef = collection(db, 'EmotionalHealth1');
        const q = query(collectionRef, orderBy('createdAt', 'desc'));
    ...
    React.useEffect(() => {
        const collectionRef = collection(db, 'EmotionalHealth2');
        const q = query(collectionRef, orderBy('createdAt', 'desc'));
    ...
    return (
        ...
        <View>
            ...
            <Text style={styles.title1}>Emotional Health</Text>
            {products.map(product => <EditEmotionalHealth key={product.id} {...product} />)}
            <TouchableOpacity onPress={() => navigation.navigate(AddEmotionalHealth)} style={styles.button}>
